 ...
 *
 * Optimized version:
 *   FIX #6: Half-rate synthesis with sample duplication.
 *   At 44100 Hz, the stock code runs ~739 sound channel iterations per
 *   frame. This is the dominant CPU cost pushing iPod Classic below 60 FPS.
 *   Fix: double snd.rate (halving iterations) and set snd.quality=2
 *   (channels advance 2 ticks per iteration). Each generated sample is
 *   written twice to the PCM buffer, maintaining 44100 Hz output.
 *   Result: ~370 iterations/frame instead of ~739, saving ~3ms/frame.
 *   For chiptune waveforms (square, noise, 4-bit PCM), the duplicated
 *   sample is perceptually identical to two independently computed ones.
 *
...
/*
 * sound_mix() -- generate PCM samples from emulated sound channels.
 *
 * FIX #6: Half-rate synthesis with sample duplication.
 *
 * At snd.rate = 94 (doubled from stock 47), the loop runs ~370 times
 * per frame instead of ~739. Each iteration generates one sample with
 * quality=2 (channels advance 2 ticks) and writes it TWICE to the
 * PCM buffer. Output: ~740 stereo pairs at 44100 Hz, same as stock.
 * CPU cost: half the channel function calls per frame.
 *
 * For chiptune waveforms (square waves, 4-bit custom wave, LFSR noise),
 * the duplicated sample is perceptually identical to computing two
 * independent samples. The maximum timing error is one sample period
 * (22.7 microseconds) -- well below audibility.
 */
void sound_mix(void)
{
    int l, r;
    ...
    for (; cpu.snd >= RATE; cpu.snd -= RATE)
    {
        ...
        if (pcm.buf)
        {
            ...
            /* Duplicate sample (2x nearest-neighbor upsample) */
            if (pcm.pos >= pcm.len)
                rockboy_pcm_submit();
            if (pcm.stereo)
            {
...
/*
 * sound_reset() -- initialize sound state.
 *
 * FIX #6: Double snd.rate and set quality=2 for half-rate synthesis.
 * The channel functions use quality to step positions and decrement
 * counters, so quality=2 makes each iteration advance 2 ticks.
 * sound_mix() writes each sample twice, so the output rate stays at
 * pcm.hz (44100) while the loop runs half as many iterations.
 */
void sound_reset(void)
{
    ...
    {
        /*
         * FIX #6: Double snd.rate for half-rate synthesis.
         *
         * Stock: snd.rate = (1<<21) / pcm.hz  => 47 at 44100 Hz
         *        snd.quality = 44100 / pcm.hz  => 1 at 44100 Hz
         *        Result: ~739 iterations/frame, quality=1
         *
         * Fixed: snd.rate = ((1<<21) / pcm.hz) * 2  => 94
         *        snd.quality = 2
         *        Result: ~370 iterations/frame, quality=2
         *        sound_mix writes each sample twice => same output rate
         */
        snd.rate = ((1<<21) / pcm.hz) * 2;
        snd.quality = 2;
    }
	else snd.rate = 0;
}
