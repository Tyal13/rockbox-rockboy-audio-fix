...
/* Rockbox plugin environment: must use rb-> API for libc functions */
#include "plugin.h"

/* Redirect libc calls to Rockbox API */
#define memset(a,b,c)   rb->memset((a),(b),(c))
#define memcpy(a,b,c)   rb->memcpy((a),(b),(c))

/* Rockbox's stdint.h may not define uint_fast types */
...
/* Configure audio format before including header */
...
ICODE_ATTR static void update_square(struct minigb_apu_ctx *ctx, audio_sample_t *samples,
		const bool ch2)
{
 ...
 */
ICODE_ATTR void minigb_apu_audio_callback(struct minigb_apu_ctx *ctx,
